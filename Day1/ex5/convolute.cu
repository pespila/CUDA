...
void gaussian_kernel(float* kernel, float sigma, int radius, int diameter) {
    ...
    for (i = -radius; i <= radius; i++) {
        for (j = -radius; j <= radius; j++) {
            kernel[(j + radius) + (i + radius) * diameter] = 1.0 / (2.0 * PI * pow(sigma, 2)) * exp((pow(i, 2) + pow(j, 2)) / (2.0 * pow(sigma, 2)));
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < repeats; i++) {
        ...
        //     {
        //         // gaussOut.at<uchar>(i, j) = h_kernel[j + i * w]*255;
        //         // printf("%2.f ", h_kernel[j + i * w]*255);
        //         // cout << (int)h_kernel[j + i * w]*255 << "   ";
        //     }
        //     cout << endl;
        // }
