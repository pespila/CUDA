...
__global__ void gamma_correction(float* out, float* in, float gamma, int width, int height) {
    int x = threadIdx.x + blockDim.x * blockIdx.x;
    int y = threadIdx.y + blockDim.y * blockIdx.y;
    int c = threadIdx.z + blockDim.z * blockIdx.z;
    int index = x + width * y + width * height * c;
    if (x < width && y < height) {
        out[index] = apply_gamma(in[index], gamma);
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < repeats; i++) {
        if (gpu) {
            gamma_correction <<<grid, block>>> (d_imgOut, d_imgIn, gamma, w, h);
        } else {
